...
*****
*****	FILE NAME: sp_XR_FrequentQueries__TEMPLATE.sql
*****
*****	PROCEDURE NAME: sp_XR_FrequentQueries
*****
...
*****
*****	PURPOSE: Returns a listing of queries (internally, StmtStoreIDs from CoreXR_SQLStmtStore
*****	ordered by frequency over a @start/@end time range, and displays important duration and wait
*****	information. What sp_XR_LongRequests is to longer-running requests (aka batches), this is to
*****	higher-frequency queries. 
*****
*****	"Frequency" here means the unique combination of SQLStmtStoreID/rqst__start_time in the SAR table.
*****	Also, the unique combination of PKInputBufferStoreID/sess__last_request_end_time is included to
*****	show frequent "pauses" in SQL activity.
******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE PROCEDURE @@CHIRHO_MASTERPROC_SCHEMA@@.sp_XR_FrequentQueries 
/*
To Execute
------------------------
...
(
	...
	@minocc			INT=15,					-- the minimum number of unique occurrences a statement or input buffer must have to qualify for the output
	...
	@context		NCHAR(1)=N'n',			--Whether or not to include the database_id from dm_exec_sessions in the grouping columns. 
...
BEGIN
	...
	SET @helpexec = N'
exec sp_XR_FrequentQueries @start=''<start datetime>'', @end=''<end datetime>'', @minocc=15, 
	...
	SELECT 
		...
		@context = LOWER(ISNULL(@context,N'z'))
		;
	...
	IF ISNULL(@minocc, -1) < 0
	BEGIN
		RAISERROR(@helpexec,10,1);
		RAISERROR('Parameter @minocc must be an integer >= 0.', 16, 1);
		RETURN -1;
	END
	...
	IF @context NOT IN (N'n', N'y')
	BEGIN
		RAISERROR(@helpexec,10,1);
		RAISERROR('Parameter @context must be either "n" (default) or "y".', 16, 1);
		RETURN -1;
	END
	...
	--We do not validate the 4 CSV filtering variables, outside of checking for NULL. We leave that to the sub-procs
	IF @dbs IS NULL
	BEGIN
		RAISERROR(@helpexec,10,1);
	...
	EXEC @@CHIRHO_DB@@.@@CHIRHO_SCHEMA_OBJECTS@@.AutoWho_ViewFrequentQueries @init = @lv__CollectionInitiatorID,
			@start = @start, 
			@end = @end, 
			@minocc = @minocc,
			@spids = @spids, 
...
-------------------------------------------------------------------------------------------------------------------------------------------
sp_XR_FrequentQueries displays data from AutoWho, a subcomponent of the ChiRho toolkit that snapshots (by default, every 15 seconds) the 
session-centric DMVs and stores the results in tables in the AutoWho schema. sp_XR_FrequentQueries searches a time window and identifies 
frequently-occurring statements and input buffers, and aggregates average/min/max data for those statements/buffers. This gives the user the 
ability to review a time window and compare the typical state of these frequent queries and any common blocking/waiting issues, resource
contention issues, or query plan issues. This proc also has cousins: 
...
different sets when using sp_XR_FrequentQueries, though only rarely will a value other than "trace" be useful.

sp_XR_FrequentQueries examines a time window (often measured in hours) and returns all unique statements (unique combinations of statement text
and request start time) and unique input buffers (unique combos of input buffer text and dm_exec_sessions.last_request_start_time) that have been 
observed between the @start and @end times. This proc can be used to compare between a time window when high-frequency activity was running well
versus when it was running poorly. Often environmental problems can be found, such as increased disk activity or tran log write times, or changes
in query plans, or increase blocking, etc.
	';
	RAISERROR(@helpstr,10,1);
...
@minocc			Valid Values: Zero or any positive integer, indicating the threshold of unique occurences required
