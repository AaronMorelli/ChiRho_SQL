...
GO
CREATE PROCEDURE   @@CHIRHO_SCHEMA_OBJECTS@@.CoreXR_ChiRhoMaster
/*
...
EXEC @@CHIRHO_SCHEMA_OBJECTS@@.CoreXR_ChiRhoMaster @ErrorMessage=@lmsg OUTPUT 
...
BEGIN
	...
	BEGIN TRY
		...
		DECLARE @lv__masterErrorString		NVARCHAR(MAX),
				...
				@lv__SQLAgentIsPresent      NCHAR(1),
				@lv__DBNameObjects          sysname;
		...
		--Some of the steps below are not applicable if SQL Agent is not present, 
		--or if this is a TempDB install. (For TempDB-only installs, even on traditional SQL Servers, we do 
		--not make use of SQL Agent jobs and instead rely on the user to start the functionality through query windows)
		SET @lv__SQLAgentIsPresent = (SELECT CASE WHEN EngineType = 'azure_sql_db' THEN N'N' ELSE N'Y' END 
									  FROM @@CHIRHO_SCHEMA_OBJECTS@@.CoreXR_InstallationConfig);
		SET @lv__DBNameObjects = (SELECT DBNameObjects FROM @@CHIRHO_SCHEMA_OBJECTS@@.CoreXR_InstallationConfig);

		IF @lv__SQLAgentIsPresent = N'Y' AND LOWER(@lv__DBNameObjects) != 'tempdb'
		BEGIN
		...
		/* comment out ServerEye functionality until we integrate into this repo
		IF @lv__SQLAgentIsPresent = N'Y' AND LOWER(@lv__DBNameObjects) != 'tempdb'
		BEGIN
		...
		*/
		...
		IF (@PurgeDOW NOT LIKE N'%NEVER%' AND (@PurgeHour IS NULL OR @PurgeHour > 24))
		BEGIN
			RAISERROR('Parameter @PurgeHour must be between 0 and 24 inclusive',16,1);
			RETURN -7;
		END

		SET @PurgeDOWHourIsUTC = UPPER(ISNULL(@PurgeDOWHourIsUTC,N'N'));
		...
		--Check if there have been any DB changes (new DBs, DBs detached/re-attached, etc) such that we need
		--to update the DB mapping
		--TODO: is this call relevant under Azure SQL DB? I suspect not, but will leave as-is (w/o a check) for now.
		...
		/*
			...
			2. After we begin (or not) each job, we run supporting procs for each:
				a. @@CHIRHO_SCHEMA_OBJECTS@@.AutoWho_UpdateStoreLastTouched
				b. @@CHIRHO_SCHEMA_OBJECTS@@.AutoWho_PostProcessor
				c. AutoWho Purge
				d. AutoWho index maintenance
				e. TODO: ServerEye_UpdateStoreLastTouched	<-- this proc does not exist yet, but likely will at some point
				f. ServerEye Purge
		...
		*/
		...
		BEGIN
			...
			EXEC @lv__ProcRC = @@CHIRHO_SCHEMA_OBJECTS@@.CoreXR_TraceTimeInfo @Utility=N'AutoWho', 
			...
			BEGIN
				...
				IF NOT EXISTS (
					...
					AND t.Running = 1)
				BEGIN
					IF NOT EXISTS (SELECT * FROM @@CHIRHO_SCHEMA_OBJECTS@@.AutoWho_SignalTable t WITH (NOLOCK) 
									WHERE LOWER(t.SignalName) = N'aborttrace' 
									AND LOWER(t.SignalValue) = N'allday'
									AND DATEDIFF(DAY, t.InsertTime, GETDATE()) = 0)	--we use local instead of UTC because the DST 1am-2am issue does not affect this logic 
					...
					BEGIN
						...
						EXEC @@CHIRHO_SCHEMA_OBJECTS@@.AutoWho_LogEvent @ProcID=@@PROCID, @EventCode = 0, @TraceID = NULL, @Location = N'XRMaster AutoWho Job Start', @Message = N'AutoWho Trace job started.';
					END
					ELSE
					BEGIN
						EXEC @@CHIRHO_SCHEMA_OBJECTS@@.AutoWho_LogEvent @ProcID=@@PROCID, @EventCode = -1, @TraceID = NULL, @Location = N'XRMaster AutoWho Signal', @Message = N'An AbortTrace signal exists for today. This procedure has been told not to run the rest of the day.';
					END
				END	 
		...
		/* Commenting out until ServerEye has been integrated into the new repo
		...
		IF @lv__SQLAgentIsPresent = N'Y' AND LOWER(@lv__DBNameObjects) != 'tempdb'
		BEGIN
			EXEC @lv__ProcRC = @@CHIRHO_SCHEMA_OBJECTS@@.CoreXR_TraceTimeInfo @Utility=N'ServerEye', 
			...
			BEGIN
				...
				IF NOT EXISTS (SELECT * 
					...
					AND t.Running = 1)
				BEGIN
					IF NOT EXISTS (SELECT * FROM @@CHIRHO_SCHEMA_OBJECTS@@.ServerEye_SignalTable t WITH (NOLOCK) 
					...
					BEGIN
						...
						EXEC @@CHIRHO_SCHEMA_OBJECTS@@.ServerEye_LogEvent @ProcID=@@PROCID, @EventCode = 0, @TraceID = NULL, @Location = N'XRMaster ServerEye Job Start', @Message = N'ServerEye Trace job started.';
					END
					ELSE
					BEGIN
						EXEC @@CHIRHO_SCHEMA_OBJECTS@@.ServerEye_LogEvent @ProcID=@@PROCID, @EventCode = -1, @TraceID = NULL, @Location = N'XRMaster ServerEye Signal', @Message = N'An AbortTrace signal exists for today. This procedure has been told not to run the rest of the day.';
					END
				END	 
		...
		*/
		...
		/*************************************** AutoWho Update Store Last Touched ***************************/
		BEGIN TRY
			SET @lv__ProcRC = 0;
			EXEC @lv__ProcRC = @@CHIRHO_SCHEMA_OBJECTS@@.AutoWho_UpdateStoreLastTouched;
		END TRY
		BEGIN CATCH
			--we swallow the error and just log it; TODO: may need to revisit this
			SET @ErrorMessage = N'Exception occurred when updating the store LastTouched values: ' + ERROR_MESSAGE();
			EXEC @@CHIRHO_SCHEMA_OBJECTS@@.AutoWho_LogEvent @ProcID=@@PROCID, @EventCode = -999, @TraceID = NULL, @Location = N'ErrorLastTouch', @Message = @ErrorMessage;
		END CATCH
		/*************************************** AutoWho Update Store Last Touched ***************************/

		/*************************************** AutoWho Postprocessor ***************************/
		BEGIN TRY
			/*
				We need to pass in valid @@CHIRHO_SCHEMA_OBJECTS@@.AutoWho_CaptureTime.UTCCaptureTime values into the post-processor. We go back 45 minutes,
			...
			*/
			...
			BEGIN
				SET @lv__ProcRC = 0;
				EXEC @lv__ProcRC = @@CHIRHO_SCHEMA_OBJECTS@@.AutoWho_PostProcessor @optionset=N'BackgroundTrace', @init=255, @startUTC=@lv__PostProcStartUTC, @endUTC=@lv__PostProcEndUTC;
			END
		END TRY
		BEGIN CATCH
			--we swallow the error and just log it; TODO: may need to revisit this
			SET @ErrorMessage = N'Exception occurred when post-processing AutoWho captures: ' + ERROR_MESSAGE();
			EXEC @@CHIRHO_SCHEMA_OBJECTS@@.AutoWho_LogEvent @ProcID=@@PROCID, @EventCode = -999, @TraceID = NULL, @Location = N'ErrorPostProcess', @Message = @ErrorMessage;
		END CATCH
		/*************************************** AutoWho Postprocessor ***************************/
		...
		IF @PurgeDOWHourIsUTC = N'N'
			...
			--AND the log doesn't show any purge as having run in the last 75 minutes
			AND NOT EXISTS (
				SELECT *
				FROM @@CHIRHO_SCHEMA_OBJECTS@@.AutoWho_Log l
		...
		BEGIN
			...
			EXEC @@CHIRHO_SCHEMA_OBJECTS@@.AutoWho_LogEvent @ProcID=@@PROCID, @EventCode = 0, @TraceID = NULL, @Location = N'XRMaster AutoWho Purge', @Message = N'Purge procedure completed';
		...
		IF @PurgeDOWHourIsUTC = N'N'
			...
			AND NOT EXISTS (
				SELECT *
				FROM @@CHIRHO_SCHEMA_OBJECTS@@.ServerEye_Log l
		...
		IF @lv__ShouldRunPurge = N'Y'
		BEGIN
			EXEC @@CHIRHO_SCHEMA_OBJECTS@@.ServerEye_ApplyRetentionPolicies;

			EXEC @@CHIRHO_SCHEMA_OBJECTS@@.ServerEye_LogEvent @ProcID=@@PROCID, @EventCode = 0, @TraceID = NULL, @Location = N'XRMaster ServerEye Purge', @Message = N'Purge procedure completed';

			--Now that we have (potentially) deleted a bunch of rows, do some index maint
			EXEC @@CHIRHO_SCHEMA_OBJECTS@@.ServerEye_MaintainIndexes;
		END
		*/
		/*************************************** ServerEye Purge ***************************/
