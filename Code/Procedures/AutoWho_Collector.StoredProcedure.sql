...
*****
*****	FILE NAME: AutoWho_Collector.StoredProcedure.sql
*****
*****	PROCEDURE NAME: AutoWho_Collector
*****
...
*****		and much of the below 4800+ lines of code are crafted in a specific fashion to reduce the impact to the SQL instance. 
*****		The AutoWho_Options table''s extended properties includes meanings for most of the parameters to this procedure, and 
*****		inline comments below will explain many other aspects of this procedure''s logic.
*****
...
GO
CREATE PROCEDURE @@CHIRHO_SCHEMA_OBJECTS@@.AutoWho_Collector
/*
	FUTURE ENHANCEMENTS: 
		...
		--Eventually optimize the Batch/Stmt query plan logic so that if we get query plans for batch, use that base data to 
		  get statement-level query plans from what we have already pulled, like what we do for the SQL batch/statement logic if
		  both are enabled.

To Execute
------------------------
<called from the AutoWho_Executor procedure>
*/
(
	--Note that validation of these variables is done by the caller (AutoWho_Executor or AutoWho_UserCollector)
...
/* Control Flow Summary
	...
	13. If we need to capture transaction info, populate the @@CHIRHO_SCHEMA_OBJECTS@@.AutoWho_TransactionDetails table with info from
		the dm_tran_* views

	14. For user spids with calc__return_to_user > 0, we join the #sar table to the SQL statement store (@@CHIRHO_SCHEMA_OBJECTS@@.CoreXR_SQLStmtStore)
		...
		@@CHIRHO_SCHEMA_OBJECTS@@.CoreXR_SQLStmtStore before. This can potentially avoid the cost of resolving lots of sql_handle/offset combos
	...
	-- This logic has been moved to the @@CHIRHO_SCHEMA_OBJECTS@@.AutoWho_PostProcessor procedure
	...
	22. We then persist the #sar data to @@CHIRHO_SCHEMA_OBJECTS@@.AutoWho_SessionsAndRequests, joining in the qp and ib tables as needed.
	...
	*/
	...
	FROM (
		...
		/* We use RIGHT OUTER JOIN and join hints to force the plan shape that we want. We want to ensure that the optimizer 
			...
			Note also the use of @@CHIRHO_SCHEMA_OBJECTS@@.AutoWho_CollectorOptFakeout. It is a simple 2 row table (values of 0 and 1) whose sole purpose
			...
			use @@CHIRHO_SCHEMA_OBJECTS@@.AutoWho_CollectorOptFakeout in that position instead, so that sys.dm_exec_sessions is pulled into a hash table
			all at once.

		*/
		FROM @@CHIRHO_SCHEMA_OBJECTS@@.AutoWho_DimWaitType dwt
			RIGHT OUTER hash JOIN @@CHIRHO_SCHEMA_OBJECTS@@.AutoWho_DimNetAddress dna
			RIGHT OUTER hash JOIN @@CHIRHO_SCHEMA_OBJECTS@@.AutoWho_DimConnectionAttribute dca
			RIGHT OUTER hash JOIN @@CHIRHO_SCHEMA_OBJECTS@@.AutoWho_DimSessionAttribute dsa

			RIGHT OUTER hash JOIN @@CHIRHO_SCHEMA_OBJECTS@@.AutoWho_DimLoginName dln
			RIGHT OUTER hash JOIN @@CHIRHO_SCHEMA_OBJECTS@@.AutoWho_DimCommand dco
							...
							RIGHT OUTER hash JOIN --#4
								...
								RIGHT OUTER hash JOIN --#5
									...
									RIGHT OUTER hash JOIN --#6
										...
										RIGHT OUTER hash JOIN 
											@@CHIRHO_SCHEMA_OBJECTS@@.AutoWho_CollectorOptFakeout cof
...
*/
	...
	FROM
		(SELECT FilterID FROM @@CHIRHO_SCHEMA_OBJECTS@@.AutoWho_CollectionFilters 
...
*/
	...
	***************************************************************************************************************************/
	...
	BEGIN
		...
		INSERT INTO @@CHIRHO_SCHEMA_OBJECTS@@.AutoWho_TransactionDetails (
	...
	*/
	...
	FROM #sessions_and_requests targ 
		INNER loop JOIN @@CHIRHO_SCHEMA_OBJECTS@@.CoreXR_SQLStmtStore sss
	...
	BEGIN
		...
		FROM #sessions_and_requests targ 
			INNER loop JOIN @@CHIRHO_SCHEMA_OBJECTS@@.CoreXR_SQLBatchStore sbs
	...
	***************************************************************************************************************************/
	...
	BEGIN
		...
		INSERT INTO @@CHIRHO_SCHEMA_OBJECTS@@.AutoWho_LockDetails (
	...
	BEGIN
		...
		WHERE NOT EXISTS (
			SELECT * FROM @@CHIRHO_SCHEMA_OBJECTS@@.AutoWho_DimCommand dc2
	...
	BEGIN
		...
		WHERE NOT EXISTS (
			SELECT * 
			FROM @@CHIRHO_SCHEMA_OBJECTS@@.AutoWho_DimConnectionAttribute dca2
	...
	BEGIN
		...
		WHERE NOT EXISTS (
			SELECT *
			FROM @@CHIRHO_SCHEMA_OBJECTS@@.AutoWho_DimLoginName dln2
	...
	BEGIN
		...
		WHERE NOT EXISTS (
			SELECT *
			FROM @@CHIRHO_SCHEMA_OBJECTS@@.AutoWho_DimNetAddress dna2
	...
	BEGIN
		...
		WHERE NOT EXISTS (
			SELECT *
			FROM @@CHIRHO_SCHEMA_OBJECTS@@.AutoWho_DimSessionAttribute dsa2
	...
	BEGIN
		...
		WHERE NOT EXISTS (
			SELECT *
			FROM @@CHIRHO_SCHEMA_OBJECTS@@.AutoWho_DimWaitType dwt2
	...
	BEGIN
		...
		-- protected by app locks). Now, we need to protect against race conditions when the Collector
		-- is running both through the Executor and by an end user
			LEFT OUTER hash JOIN @@CHIRHO_SCHEMA_OBJECTS@@.AutoWho_DimWaitType dwt
	...
	BEGIN
		...
		IF @NewDims__Command > 0
		BEGIN
			SET @lv__BigNvar = @lv__BigNvar + N' @@CHIRHO_SCHEMA_OBJECTS@@.AutoWho_DimCommand dco RIGHT OUTER hash JOIN
			';
		END

		IF @NewDims__ConnectionAttribute > 0
		BEGIN
			SET @lv__BigNvar = @lv__BigNvar + N' @@CHIRHO_SCHEMA_OBJECTS@@.AutoWho_DimConnectionAttribute dca RIGHT OUTER hash JOIN
			';
		END

		IF @NewDims__LoginName > 0
		BEGIN
			SET @lv__BigNvar = @lv__BigNvar + N' @@CHIRHO_SCHEMA_OBJECTS@@.AutoWho_DimLoginName dln RIGHT OUTER hash JOIN
			';
		END

		IF @NewDims__NetAddress > 0
		BEGIN
			SET @lv__BigNvar = @lv__BigNvar + N' @@CHIRHO_SCHEMA_OBJECTS@@.AutoWho_DimNetAddress dna RIGHT OUTER hash JOIN
			';
		END

		IF @NewDims__SessionAttribute > 0
		BEGIN
			SET @lv__BigNvar = @lv__BigNvar + N' @@CHIRHO_SCHEMA_OBJECTS@@.AutoWho_DimSessionAttribute dsa RIGHT OUTER hash JOIN
			';
		END

		IF @NewDims__WaitType_sar > 0
		BEGIN
			SET @lv__BigNvar = @lv__BigNvar + N' @@CHIRHO_SCHEMA_OBJECTS@@.AutoWho_DimWaitType dwt RIGHT OUTER hash JOIN
			';
		END
	...
	BEGIN
		...
		INSERT INTO @@CHIRHO_SCHEMA_OBJECTS@@.AutoWho_SARException(
	...
	BEGIN
		...
		MERGE @@CHIRHO_SCHEMA_OBJECTS@@.CoreXR_SQLStmtStore perm
			USING #t__stmt t
		...
		FROM #sessions_and_requests sar
			INNER hash JOIN @@CHIRHO_SCHEMA_OBJECTS@@.CoreXR_SQLStmtStore sss
	...
	BEGIN
		...
		MERGE @@CHIRHO_SCHEMA_OBJECTS@@.CoreXR_SQLBatchStore perm
			USING #t__batch t
				ON perm.sql_handle = t.sql_handle
		WHEN MATCHED THEN UPDATE
		...
		FROM #sessions_and_requests sar
			INNER hash JOIN @@CHIRHO_SCHEMA_OBJECTS@@.CoreXR_SQLBatchStore bs
	...
	BEGIN
		...
		FROM #t__ib t
			INNER JOIN @@CHIRHO_SCHEMA_OBJECTS@@.CoreXR_InputBufferStore s
	...
	BEGIN
		...
		--INSERT INTO @@CHIRHO_SCHEMA_OBJECTS@@.CoreXR_QueryPlanBatchStore
		...
		--WHERE NOT EXISTS (
		--	SELECT *
		--	FROM @@CHIRHO_SCHEMA_OBJECTS@@.CoreXR_QueryPlanBatchStore bs2
		...
		MERGE @@CHIRHO_SCHEMA_OBJECTS@@.CoreXR_QueryPlanBatchStore perm
			USING #t__batchqp t
		...
		FROM #t__batchqp t
			INNER JOIN @@CHIRHO_SCHEMA_OBJECTS@@.CoreXR_QueryPlanBatchStore bs
	...
	BEGIN
		...
		--INSERT INTO @@CHIRHO_SCHEMA_OBJECTS@@.CoreXR_QueryPlanStmtStore 
		...
		--WHERE NOT EXISTS (
		--	SELECT * 
		--	FROM @@CHIRHO_SCHEMA_OBJECTS@@.CoreXR_QueryPlanStmtStore qps
		...
		MERGE @@CHIRHO_SCHEMA_OBJECTS@@.CoreXR_QueryPlanStmtStore perm
			USING #t__stmtqp t
		...
		*/
		...
		FROM #t__stmtqp t
			INNER JOIN @@CHIRHO_SCHEMA_OBJECTS@@.CoreXR_QueryPlanStmtStore qps
	...
	SET @lv__BigNvar = N'
	INSERT INTO @@CHIRHO_SCHEMA_OBJECTS@@.AutoWho_SessionsAndRequests 
	(	CollectionInitiatorID,			--1
		UTCCaptureTime,
		SPIDCaptureTime,
		session_id, 
	...
	INSERT INTO @@CHIRHO_SCHEMA_OBJECTS@@.AutoWho_TasksAndWaits (
		CollectionInitiatorID,
		UTCCaptureTime,
		SPIDCaptureTime, 
		task_address, 
	...
	INSERT INTO @@CHIRHO_SCHEMA_OBJECTS@@.AutoWho_CaptureTimes (
		[CollectionInitiatorID],
		[SPIDCaptureTime],
		[UTCCaptureTime],
		[RunWasSuccessful],
...
BEGIN CATCH
	...
	INSERT INTO @@CHIRHO_SCHEMA_OBJECTS@@.AutoWho_TAWException (
