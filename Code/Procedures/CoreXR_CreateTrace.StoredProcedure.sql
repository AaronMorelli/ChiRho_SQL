...
GO
CREATE PROCEDURE @@CHIRHO_SCHEMA_OBJECTS@@.CoreXR_CreateTrace
/*
		@Utility is one of these values: "AutoWho", "ServerEye", "Profiler"
...
------------------------
DECLARE @TraceID INT;
EXEC @TraceID = @@CHIRHO_SCHEMA_OBJECTS@@.CoreXR_CreateTrace @Utility=N'AutoWho', @Type=N'Background', @IntendedStopTime='2016-04-24 23:59'
*/
(
	@Utility			NVARCHAR(20),
	@Type				NVARCHAR(20),
...
BEGIN
	...
	IF UPPER(ISNULL(@Type,N'null')) != 'BACKGROUND'
	BEGIN
		RAISERROR('Parameter @Type must currently always be set to "Background".',16,1);
		RETURN -1;
	END
	...
	BEGIN TRY
		...
		INSERT INTO @@CHIRHO_SCHEMA_OBJECTS@@.CoreXR_Traces
		...
		IF ISNULL(@LastIdentity, -1) <= 0
		BEGIN
			RAISERROR('Invalid value for @LastIdentity', 16, 1);
			RETURN -1;
		END
