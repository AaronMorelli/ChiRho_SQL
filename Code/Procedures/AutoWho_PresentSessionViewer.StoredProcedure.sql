...
*****
*****	FILE NAME: AutoWho_PresentSessionViewer.StoredProcedure.sql
*****
*****	PROCEDURE NAME: AutoWho_PresentSessionViewer
*****
...
*****
*****	PURPOSE: Called by the sp_XR_SessionViewer user-facing procedure when historical/AutoWho data is requested. 
...
GO
CREATE PROCEDURE @@CHIRHO_SCHEMA_OBJECTS@@.AutoWho_PresentSessionViewer
/*
	FUTURE ENHANCEMENTS: 
		...
		- If the "self" spid is captured in AutoWho.Collector, there is not a way to identify it in the data.
			Thus, even if we added a "show self" option in this historical view, we would not know which spid
...
EXEC @@CHIRHO_SCHEMA_OBJECTS@@.AutoWho_PresentSessionViewer @htcUTC='2016-04-25 16:12', @hct='2016-04-25 08:12',
...
BEGIN
	...
	/*
		...
		For historical runs, we can just use the simple 1/0 flags that are stored in @@CHIRHO_SCHEMA_OBJECTS@@.AutoWho_CaptureSummary by the 
		@@CHIRHO_SCHEMA_OBJECTS@@.AutoWho_PopulateCaptureSummary procedure (which looks at the base data in the AutoWho tables to determine these bit flag values). 
		Thus, pull those values.

		For current runs, @@CHIRHO_SCHEMA_OBJECTS@@.AutoWho_CaptureSummary doesn't have any data in it, so we can't check it to see if bchain data was
	...
	BEGIN
		...
		FROM (SELECT 1 as col1) t
			OUTER APPLY (
				...
				FROM @@CHIRHO_SCHEMA_OBJECTS@@.AutoWho_BlockingGraphs b
				WHERE b.CollectionInitiatorID = @init
				AND b.UTCCaptureTime = @hctUTC
			) b
			OUTER APPLY (
				SELECT TOP 1 l.UTCCaptureTime
				FROM @@CHIRHO_SCHEMA_OBJECTS@@.AutoWho_LockDetails l
	...
	BEGIN
		...
		BEGIN
			...
			SELECT convert(xml,N'<?optqry --' + NCHAR(10) +
						...
						N' -- ' + NCHAR(10) + NCHAR(13) + 
						N'SELECT * FROM @@CHIRHO_SCHEMA_OBJECTS@@.AutoWho_CaptureTimes ct WITH (NOLOCK) ORDER BY ct.UTCCaptureTime DESC;
SELECT * FROM @@CHIRHO_SCHEMA_OBJECTS@@.AutoWho_Log l WITH (NOLOCK) ORDER BY l.LogDT DESC;
SELECT * FROM @@CHIRHO_SCHEMA_OBJECTS@@.AutoWho_SessionsAndRequests sar WITH (NOLOCK) WHERE sar.UTCCaptureTime = ''' + 
...
SELECT * FROM @@CHIRHO_SCHEMA_OBJECTS@@.AutoWho_TasksAndWaits taw WITH (NOLOCK) WHERE taw.UTCCaptureTime = ''' + 
	REPLACE(CONVERT(NVARCHAR(20), @hctUTC, 102),'.','-')+' '+CONVERT(NVARCHAR(20), @hctUTC, 108)+'.'+RIGHT(CONVERT(NVARCHAR(20),N'000')+CONVERT(NVARCHAR(20),DATEPART(MILLISECOND, @hctUTC)),3)+N''';
SELECT * FROM @@CHIRHO_SCHEMA_OBJECTS@@.AutoWho_TransactionDetails td WITH (NOLOCK) WHERE td.UTCCaptureTime = ''' + 
	REPLACE(CONVERT(NVARCHAR(20), @hctUTC, 102),'.','-')+' '+CONVERT(NVARCHAR(20), @hctUTC, 108)+'.'+RIGHT(CONVERT(NVARCHAR(20),N'000')+CONVERT(NVARCHAR(20),DATEPART(MILLISECOND, @hctUTC)),3)+N''';
SELECT * FROM @@CHIRHO_SCHEMA_OBJECTS@@.AutoWho_LockDetails ld WITH (NOLOCK) WHERE ld.UTCCaptureTime = ''' + 
	REPLACE(CONVERT(NVARCHAR(20), @hctUTC, 102),'.','-')+' '+CONVERT(NVARCHAR(20), @hctUTC, 108)+'.'+RIGHT(CONVERT(NVARCHAR(20),N'000')+CONVERT(NVARCHAR(20),DATEPART(MILLISECOND, @hctUTC)),3)+N''';
SELECT * FROM AutoWho.BlockingGraphs bg WITH (NOLOCK) WHERE bg.UTCCaptureTime = ''' + 
	REPLACE(CONVERT(NVARCHAR(20), @hctUTC, 102),'.','-')+' '+CONVERT(NVARCHAR(20), @hctUTC, 108)+'.'+RIGHT(CONVERT(NVARCHAR(20),N'000')+CONVERT(NVARCHAR(20),DATEPART(MILLISECOND, @hctUTC)),3)+N''';
SELECT * FROM @@CHIRHO_SCHEMA_OBJECTS@@.CoreXR_SQLStmtStore sss WITH (NOLOCK) WHERE sss.PKSQLStmtStoreID = ;
SELECT * FROM @@CHIRHO_SCHEMA_OBJECTS@@.CoreXR_QueryPlanStmtStore qpss WITH (NOLOCK) WHERE qpss.PKQueryPlanStmtStoreID = ;
SELECT dco.* --,sar.* 
FROM @@CHIRHO_SCHEMA_OBJECTS@@.AutoWho_DimCommand dco --INNER JOIN @@CHIRHO_SCHEMA_OBJECTS@@.AutoWho_SessionsAndRequests sar WITH (NOLOCK) ON sar.rqst__FKDimCommand = dco.DimCommandID
SELECT dca.* --,sar.* 
FROM @@CHIRHO_SCHEMA_OBJECTS@@.AutoWho_DimConnectionAttribute dca --INNER JOIN @@CHIRHO_SCHEMA_OBJECTS@@.AutoWho_SessionsAndRequests sar WITH (NOLOCK) ON sar.conn__FKDimConnectionAttribute = dca.DimConnectionAttributeID
SELECT dln.* --,sar.* 
FROM @@CHIRHO_SCHEMA_OBJECTS@@.AutoWho_DimLoginName dln --INNER JOIN @@CHIRHO_SCHEMA_OBJECTS@@.AutoWho_SessionsAndRequests sar WITH (NOLOCK) ON sar.sess__FKDimLoginName = dln.DimLoginNameID
SELECT dna.* --,sar.* 
FROM @@CHIRHO_SCHEMA_OBJECTS@@.AutoWho_DimNetAddress dna  --INNER JOIN @@CHIRHO_SCHEMA_OBJECTS@@.AutoWho_SessionsAndRequests sar WITH (NOLOCK) ON sar.conn__FKDimNetAddress = dna.DimNetAddressID
SELECT dsa.*  --,sar.* 
FROM @@CHIRHO_SCHEMA_OBJECTS@@.AutoWho_DimSessionAttribute dsa --INNER JOIN @@CHIRHO_SCHEMA_OBJECTS@@.AutoWho_SessionsAndRequests sar WITH (NOLOCK) ON sar.sess__FKDimSessionAttribute = dsa.DimSessionAttributeID
SELECT dwt.*  --,sar.* 
FROM @@CHIRHO_SCHEMA_OBJECTS@@.AutoWho_DimWaitType dwt  --INNER JOIN @@CHIRHO_SCHEMA_OBJECTS@@.AutoWho_SessionsAndRequests sar WITH (NOLOCK) ON sar.rqst__FKDimWaitType = dwt.DimWaitTypeID
SELECT dwt.* ,taw.* 
FROM @@CHIRHO_SCHEMA_OBJECTS@@.AutoWho_DimWaitType dwt INNER JOIN @@CHIRHO_SCHEMA_OBJECTS@@.AutoWho_TasksAndWaits taw WITH (NOLOCK) ON taw.FKDimWaitType = dwt.DimWaitTypeID' + NCHAR(10) + NCHAR(13) + N'--?>')
		END
	END
	...
	BEGIN
		...
		-- collected. For performance reasons, we do not resolve DBID in AutoWho.Collector; thus, if a DB is detached/re-attached,
	...
	BEGIN
		...
		SELECT 
			...
			--sbs.objectid,
			sbs.batch_text
		FROM @@CHIRHO_SCHEMA_OBJECTS@@.CoreXR_SQLBatchStore sbs
		WHERE EXISTS (
			SELECT *
			FROM #DistinctFKs fk
		...
		WHILE @@FETCH_STATUS = 0
		BEGIN
			/* We do not need to do this logic ("what is dbname? what is object name?") for the batch because we already did it for the statement
			IF @dbid > 0
			BEGIN
				SET @dbname = DB_NAME(@dbid);
			END
	...
	BEGIN
		...
		SELECT 
			...
			qpbs.plan_handle,
			qpbs.query_plan
		FROM @@CHIRHO_SCHEMA_OBJECTS@@.CoreXR_QueryPlanBatchStore qpbs
		WHERE EXISTS (
			SELECT *
			FROM #DistinctFKs fk
				...
				BEGIN
					...
					BEGIN CATCH
						...
						CASE WHEN ERROR_NUMBER() = 6335 AND @PKSQLStmtStoreID IS NOT NULL THEN 
							...
							SELECT query_plan
							INTO dbo.largeQPbcpout
							FROM @@CHIRHO_SCHEMA_OBJECTS@@.CoreXR_QueryPlanBatchStore q
		...
		BEGIN TRY
			...
			FROM (
								...
								CASE WHEN bg.request_id = @lv__nullsmallint
										THEN ( 
											...
											(SELECT 1 as col1) dummy1 
												...
												FROM @@CHIRHO_SCHEMA_OBJECTS@@.CoreXR_InputBufferStore b
				...
				FROM @@CHIRHO_SCHEMA_OBJECTS@@.AutoWho_BlockingGraphs bg
					LEFT OUTER JOIN @@CHIRHO_SCHEMA_OBJECTS@@.CoreXR_SQLStmtStore stmt
			...
			--select * from @@CHIRHO_SCHEMA_OBJECTS@@.AutoWho_BlockingGraphs order by UTCCaptureTime 
			...
			FROM (
				...
				FROM (
					...
					FROM (
						...
						FROM (
							SELECT
							--select * from @@CHIRHO_SCHEMA_OBJECTS@@.AutoWho_TransactionDetails
								session_id,
								dtat_transaction_id,
								dtat_name,
	...
	;WITH basedata AS (
		...
		*/
		...
		BEGIN
			...
			wait_type = CASE WHEN sar.session_id < 0 OR sar.request_id = @lv__nullsmallint THEN N'''' 
				WHEN sar.calc__sysspid_isinteresting = 1 THEN (SELECT ' + CASE WHEN @savespace = N'Y' THEN N'wait_type_short' ELSE N'wait_type' END + N'
							FROM @@CHIRHO_SCHEMA_OBJECTS@@.AutoWho_DimWaitType syswait
		...
		SET @lv__BaseFROM = @lv__BaseFROM + N'
			LEFT OUTER JOIN @@CHIRHO_SCHEMA_OBJECTS@@.AutoWho_DimCommand dimcmd
				ON sar.rqst__FKDimCommand = dimcmd.DimCommandID
			LEFT OUTER JOIN @@CHIRHO_SCHEMA_OBJECTS@@.AutoWho_TasksAndWaits taw
