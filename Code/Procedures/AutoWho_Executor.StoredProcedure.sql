...
*****
*****	FILE NAME: AutoWho_Executor.StoredProcedure.sql
*****
*****	PROCEDURE NAME: AutoWho_Executor
*****
...
GO
CREATE PROCEDURE @@CHIRHO_SCHEMA_OBJECTS@@.AutoWho_Executor
/*
To stop the trace before its end-time: 
	exec @@CHIRHO_SCHEMA_OBJECTS@@.CoreXR_AbortTrace @Utility=N'AutoWho',@TraceID = NULL | <number>, @PreventAllDay = N'N' | N'Y'		--null trace ID means the most recent one


DECLARE @ProcRC INT
DECLARE @lmsg VARCHAR(4000)

EXEC @ProcRC = @@CHIRHO_SCHEMA_OBJECTS@@.AutoWho_Executor @ErrorMessage = @lmsg OUTPUT
...
/* Control Flow Summary
	Here's the work done by this proc:
		...
		6. Determine whether there are any long-running SPIDs that we don't want to interfere with the Collector's
			...
			the @@CHIRHO_SCHEMA_OBJECTS@@.AutoWho_ThresholdFilterSpids

		7. Call the @@CHIRHO_SCHEMA_OBJECTS@@.AutoWho_PrePopulateDimensions table to add any dim keys that might not already be there.
		...
		11. Determines when the last time the Store tables had their "LastTouchedBy" field updated, via the @@CHIRHO_SCHEMA_OBJECTS@@.AutoWho_Log table. 
			Then, we call the @@CHIRHO_SCHEMA_OBJECTS@@.AutoWho_UpdateStoreLastTouched proc to get the Stores fully up-to-date before we start the collector.
		...
		The loop's logic is this: 
			...
			b. Call the Collector procedure with all of our option parameters and the filter table.
				i. If it is time to recompile the @@CHIRHO_SCHEMA_OBJECTS@@.AutoWho_Collector procedure, we call the proc WITH RECOMPILE
			...
			g. Every @opt__ThresholdFilterRefresh times through the loop, empty the @@CHIRHO_SCHEMA_OBJECTS@@.AutoWho_ThresholdFilterSpids table and re-calculate.
			...
			i. If it has been 10 minutes since we last called @@CHIRHO_SCHEMA_OBJECTS@@.AutoWho_UpdateStoreLastTouched, do that now.
...
BEGIN TRY
	...
	BEGIN
		...
		EXEC @@CHIRHO_SCHEMA_OBJECTS@@.AutoWho_LogEvent @ProcID=@@PROCID, @EventCode=-1, @TraceID=NULL, @Location='SQLVersion', @Message=@ErrorMessage; 
		RETURN -1;
	END
	...
	BEGIN
		...
		SET @lv__ThisRC = -3;
		EXEC @@CHIRHO_SCHEMA_OBJECTS@@.AutoWho_LogEvent @ProcID=@@PROCID, @EventCode=@lv__ThisRC, @TraceID=NULL, @Location='Obtaining applock', @Message=@ErrorMessage;
		RETURN @lv__ThisRC;
	END
	...
	BEGIN
		...
		SET @lv__ThisRC = -5;
		EXEC @@CHIRHO_SCHEMA_OBJECTS@@.AutoWho_LogEvent @ProcID=@@PROCID, @EventCode=@lv__ThisRC, @TraceID=NULL, @Location='Perms Validation', @Message=@ErrorMessage;
	...
	IF EXISTS (SELECT * FROM @@CHIRHO_SCHEMA_OBJECTS@@.AutoWho_SignalTable WITH (ROWLOCK) 
	...
	BEGIN
		...
		SET @lv__ThisRC = -9;
		EXEC @@CHIRHO_SCHEMA_OBJECTS@@.AutoWho_LogEvent @ProcID=@@PROCID, @EventCode=@lv__ThisRC, @TraceID=NULL, @Location='Abort flag exists', @Message=@ErrorMessage;
	...
	EXEC @@CHIRHO_SCHEMA_OBJECTS@@.CoreXR_TraceTimeInfo @Utility=N'AutoWho', @PointInTimeUTC = NULL, @UtilityIsEnabled = @lv__AutoWhoEnabled OUTPUT,
	...
	BEGIN
		...
		SET @lv__ThisRC = -11;
		EXEC @@CHIRHO_SCHEMA_OBJECTS@@.AutoWho_LogEvent @ProcID=@@PROCID, @EventCode=@lv__ThisRC, @TraceID=NULL, @Location='NotEnabled', @Message=@ErrorMessage;
	...
	BEGIN
		...
		SET @lv__ThisRC = -13;
		EXEC @@CHIRHO_SCHEMA_OBJECTS@@.AutoWho_LogEvent @ProcID=@@PROCID, @EventCode=@lv__ThisRC, @TraceID=NULL, @Location='Outside Begin/End', @Message=@ErrorMessage;
	...
	*/
	...
	BEGIN
		BEGIN TRY 
			INSERT INTO @@CHIRHO_SCHEMA_OBJECTS@@.AutoWho_CollectionFilters (CollectionInitiatorID, FilterType, FilterID, FilterName)
		...
		BEGIN CATCH
			...
			SET @lv__ThisRC = -15;
			EXEC @@CHIRHO_SCHEMA_OBJECTS@@.AutoWho_LogEvent @ProcID=@@PROCID, @EventCode=@lv__ThisRC, @TraceID=NULL, @Location='DB Inclusions', @Message=@ErrorMessage;
	...
	BEGIN
		...
		BEGIN CATCH
			...
			SET @lv__ThisRC = -17;
			EXEC @@CHIRHO_SCHEMA_OBJECTS@@.AutoWho_LogEvent @ProcID=@@PROCID, @EventCode=@lv__ThisRC, @TraceID=NULL, @Location='DB Exclusions', @Message=@ErrorMessage;
	...
	INSERT INTO @@CHIRHO_SCHEMA_OBJECTS@@.AutoWho_CollectionFilters (CollectionInitiatorID, FilterType, FilterID)
	...
	IF EXISTS (SELECT * 
		FROM @@CHIRHO_SCHEMA_OBJECTS@@.AutoWho_CollectionFilters t1 
			INNER JOIN @@CHIRHO_SCHEMA_OBJECTS@@.AutoWho_CollectionFilters t2 
	...
	BEGIN
		...
		SET @lv__ThisRC = -19;
		EXEC @@CHIRHO_SCHEMA_OBJECTS@@.AutoWho_LogEvent @ProcID=@@PROCID, @EventCode=@lv__ThisRC, @TraceID=NULL, @Location='IncludeExclude', @Message=@ErrorMessage;
	...
	BEGIN
		...
		SET @lv__ThisRC = -21;
		EXEC @@CHIRHO_SCHEMA_OBJECTS@@.AutoWho_LogEvent @ProcID=@@PROCID, @EventCode=@lv__ThisRC, @TraceID=NULL, @Location='Less 60sec', @Message=@ErrorMessage;
	...
	BEGIN TRY
		...
		BEGIN
			...
			SET @lv__ThisRC = -23;
			EXEC @@CHIRHO_SCHEMA_OBJECTS@@.AutoWho_LogEvent @ProcID=@@PROCID, @EventCode=@lv__ThisRC, @TraceID=NULL, @Location='InvalidTraceID', @Message=@ErrorMessage;
	...
	BEGIN CATCH
		...
		SET @lv__ThisRC = -25;
		EXEC @@CHIRHO_SCHEMA_OBJECTS@@.AutoWho_LogEvent @ProcID=@@PROCID, @EventCode=@lv__ThisRC, @TraceID=NULL, @Location='CreateTraceException', @Message=@ErrorMessage;
	...
	EXEC @@CHIRHO_SCHEMA_OBJECTS@@.AutoWho_LogEvent @ProcID=@@PROCID, @EventCode=0, @TraceID=@lv__TraceID, @Location='Print TraceID', @Message=@ErrorMessage;

	SET @ErrorMessage = N'The AutoWho trace is going to run for ''' + convert(varchar(20),@lv__RunTimeSeconds) + ''' seconds.';
	EXEC @@CHIRHO_SCHEMA_OBJECTS@@.AutoWho_LogEvent @ProcID=@@PROCID, @EventCode=0, @TraceID=@lv__TraceID, @Location='Runtime calc', @Message=@ErrorMessage;
	...
	-- we need to handle them. Most of the time, we will use @lv__TempDBCreateTime as our fall-back value
	...
	BEGIN
		...
		BEGIN
			...
			BEGIN CATCH
				...
				SET @lv__ThisRC = -31;
				EXEC @@CHIRHO_SCHEMA_OBJECTS@@.AutoWho_LogEvent @ProcID=@@PROCID, @EventCode=@lv__ThisRC, @TraceID=@lv__TraceID, @Location='TF8666Enable', @Message=@ErrorMessage;
	...
	SET @lv__RecompileAutoWho = 1;		--lets start off by recompiling the proc
	...
	BEGIN
		...
		BEGIN CATCH
			...
			EXEC @@CHIRHO_SCHEMA_OBJECTS@@.AutoWho_LogEvent @ProcID=@@PROCID, @EventCode=-33, @TraceID=@lv__TraceID, @Location='Executor: AutoWho Collector exception', @Message=@ErrorMessage;

			SET @lv__SuccessiveExceptions = @lv__SuccessiveExceptions + 1;

			IF @lv__SuccessiveExceptions >= 10
			BEGIN
				EXEC @@CHIRHO_SCHEMA_OBJECTS@@.AutoWho_LogEvent @ProcID=@@PROCID, @EventCode=-35, @TraceID=@lv__TraceID, @Location='Abort b/c exceptions', @Message=N'10 consecutive failures; this procedure is terminating.';
		...
		BEGIN
			...
			EXEC @@CHIRHO_SCHEMA_OBJECTS@@.AutoWho_LogEvent @ProcID=@@PROCID, @EventCode=0, @TraceID=@lv__TraceID, @Location='LightColl', @Message=@ErrorMessage;
		END
		...
		BEGIN
			...
			BEGIN
				...
				BEGIN
					...
					EXEC @@CHIRHO_SCHEMA_OBJECTS@@.AutoWho_LogEvent @ProcID=@@PROCID, @EventCode=0, @TraceID=@lv__TraceID, @Location='Recompile', @Message=@ErrorMessage;
				END
			END
		...
		BEGIN
		 	DELETE FROM @@CHIRHO_SCHEMA_OBJECTS@@.AutoWho_CollectionFilters WHERE CollectionInitiatorID = 255 AND FilterType = 128;
		...
		FROM (
			SELECT TOP 1 
				CASE WHEN LOWER(SignalValue) = N'allday' THEN N'A' 
					...
					ELSE NULL 
					END as firstchar
			FROM @@CHIRHO_SCHEMA_OBJECTS@@.AutoWho_SignalTable WITH (NOLOCK) 
		...
		BEGIN
			...
			BEGIN
				...
				EXEC @@CHIRHO_SCHEMA_OBJECTS@@.AutoWho_LogEvent @ProcID=@@PROCID, @EventCode=1, @TraceID=@lv__TraceID, @Location='Abort b/c signal', @Message=@ErrorMessage;
			END
		END
	...
	BEGIN
		...
		EXEC @@CHIRHO_SCHEMA_OBJECTS@@.AutoWho_LogEvent @ProcID=@@PROCID, @EventCode=@lv__ThisRC, @TraceID=@lv__TraceID, @Location='Exception exit', @Message=@ErrorMessage;

		EXEC @@CHIRHO_SCHEMA_OBJECTS@@.CoreXR_AbortTrace @Utility = N'AutoWho', @TraceID = @lv__TraceID, @AbortCode = @lv__EarlyAbort, @PreventAllDay = N'Y';
	END
	ELSE IF @lv__EarlyAbort IN (N'O', N'A')
	BEGIN
		SET @lv__ThisRC = -39;
		SET @ErrorMessage = 'Exiting wrapper procedure due to manual abort, type: ' + @lv__EarlyAbort;
		EXEC @@CHIRHO_SCHEMA_OBJECTS@@.AutoWho_LogEvent @ProcID=@@PROCID, @EventCode=@lv__ThisRC, @TraceID=@lv__TraceID, @Location='Manual abort exit', @Message=@ErrorMessage;
	...
	BEGIN
		...
		EXEC @@CHIRHO_SCHEMA_OBJECTS@@.AutoWho_LogEvent @ProcID=@@PROCID, @EventCode=@lv__ThisRC, @TraceID=@lv__TraceID, @Location='Successful complete', @Message=@ErrorMessage;

		EXEC @@CHIRHO_SCHEMA_OBJECTS@@.CoreXR_StopTrace @Utility=N'AutoWho', @TraceID = @lv__TraceID, @AbortCode = @lv__EarlyAbort;
	END
	...
	BEGIN
		...
		BEGIN
			...
			BEGIN CATCH
				...
				SET @lv__ThisRC = -43;
				EXEC @@CHIRHO_SCHEMA_OBJECTS@@.AutoWho_LogEvent @ProcID=@@PROCID, @EventCode=@lv__ThisRC, @TraceID=@lv__TraceID, @Location='TF8666Disable', @Message=@ErrorMessage;
...
BEGIN CATCH
	...
	EXEC @@CHIRHO_SCHEMA_OBJECTS@@.AutoWho_LogEvent @ProcID=@@PROCID, @EventCode=-999, @TraceID=NULL, @Location='CATCH block', @Message=@ErrorMessage;
